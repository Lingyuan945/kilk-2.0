            ...
            <div key={i} className={`animate-fade-in-up stagger-delay-${i + 1}`}>
              <Card className="glass-card glass-card-hover h-full hover-lift">
                <CardContent className="p-3 md:p-6">
                  <h3 className="mb-2 text-[11px] font-semibold text-slate-200 md:mb-3 md:text-sm">{stack.category}</h3>
                  <div className="flex flex-wrap gap-1.5 md:gap-2">
                    {stack.items.map((item, j) => (
                      <span
                        key={j}
                        className="rounded-md border border-slate-700/70 bg-slate-800/50 px-1.5 py-0.5 text-[10px] font-medium text-slate-300 transition-colors hover:border-blue-500/40 hover:bg-blue-500/10 hover:text-blue-300 md:px-2.5 md:py-1 md:text-xs"
                      >
                        {item}
                      </span>
                    ))}
